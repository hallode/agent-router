...
router_activity_record "$EV" "$SID" "$M"
[ "$EV" = SessionEnd ] && exit 0
if [ "$EV" = PostModelSwitch ]; then
  FROM=$(printf '%s' "$IN" | jq -r '.from_model // ""' 2>/dev/null)
  if [ -n "$FROM" ] && [ -n "$M" ] && [ "$FROM" != "$M" ]; then
    jq -nc --arg from "$FROM" --arg to "$M" \
      '{systemMessage:("Agent Router · Claude switched model: " + $from + " → " + $to + ".")}'
  fi
fi
[ -n "$M" ] || exit 0
...
printf '%s' "$M" > "$SESS/$SID.model" 2>/dev/null || true
